...
double drhodr_profile_nfwcutoff(double r, double rc_param, double nt_nfw_scaler, double falloff_C_param) { // Added falloff_C_param
    ...
    if (C_cutoff_factor <= 0)
        C_cutoff_factor = 19.0; // Safety default
    ...
    if (term_s <= 1e-9)
        term_s = 1e-9; // Avoid division by zero for denominator
...
double fEintegrand_nfw(double t_integration_var, void *params) {
    ...
    if (spline_x_input_rPsi < spline_rPsi_x_min)
        spline_x_input_rPsi = spline_rPsi_x_min;
    if (spline_x_input_rPsi > spline_rPsi_x_max)
        spline_x_input_rPsi = spline_rPsi_x_max;
    ...
    if (M_at_r_val < 0)
        M_at_r_val = 0; // Mass must be non-negative
    ...
    if (!isfinite(integrand_value)) {
        if (g_doDebug)
            fprintf(stderr, "Warning: NFW fEintegrand (refactored) returning non-finite value for t_in=%.3e, E_shell=%.3e\n", t_integration_var, E_shell);
...
double massintegrand_profile_nfwcutoff(double r, void *params) {
    ...
    if (C_cutoff_factor <= 0)
        C_cutoff_factor = 19.0; // Safety default if param is bad
    ...
    if (term_softening <= 1e-9)
        term_softening = 1e-9; // Avoid division by zero from softening
