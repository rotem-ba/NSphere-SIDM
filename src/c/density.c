...
/**
 * @brief Calculates drho/dPsi for the selected density profile.
 * @details Calcualtes the value at the point radius[i].
 */
double calculate_density_drhodPsi(int i) {
    if (g_use_nfw_profile) {
        ...
        double dPsidr_mag_k = (G_CONST * Menc_k) / sqr(rr);

        if (fabs(dPsidr_mag_k) > 1e-30)
            return drhodr_val_k / dPsidr_mag_k;
    }
    else {
        double rr = radius[i];
